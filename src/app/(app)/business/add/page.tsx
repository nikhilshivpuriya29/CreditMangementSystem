...
import { Loader2, Building, User, Phone, MapPin, Upload, Info } from "lucide-react";
...
export default function AddBusinessPage() {
  ...
  async function onSubmit(values: z.infer<typeof BusinessSchema>) {
    setIsLoading(true);
    console.log("Adding Business:", values);

    // --- Mock Business Save ---
    await new Promise((resolve) => setTimeout(resolve, 1500)); // Simulate network delay
     toast({
      title: "Business Profile Added",
      description: `${values.businessName} has been added successfully.`,
    });
    // --- End Mock ---

    setIsLoading(false);
    // Redirect to business management page or dashboard
     router.push('/business');
     // Alternatively: router.push('/dashboard');
  }
    ...
    <Card className="w-full max-w-2xl mx-auto shadow-lg">
      <CardHeader>
        <CardTitle className="text-xl flex items-center gap-2">
           <Building className="h-5 w-5" /> Add New Business Profile
        </CardTitle>
        <CardDescription>
          Create a separate profile for each of your businesses.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
             ...
             <FormField
              ...
              render={({ field }) => (
                 <FormItem>
                   ...
                   <FormControl>
                       {/* Basic file input - enhance later with preview */}
